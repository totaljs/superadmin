...
Component: j-Gauge
Version: 1
Updated: 2023-01-31T10:17:00.000Z
Author: Peter Širka
...
Component: j-Layout2
Version: 1
Updated: 2023-01-31T14:25:06.000Z
Author: Peter Širka


Component: j-LiveStats
Version: 1
Updated: 2023-01-31T14:00:01.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-01-31T15:00:01.000Z
Author: Peter Širka


Component: j-Menu
Version: 1
Updated: 2023-12-30T23:48:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2022-02-01T10:25:00.001Z
Author: Denis Granec
...
.ui-gauge text{font-family:Arial;fill:#000;font-weight:bold}.ui-gauge-pointer path{fill:#000}.ui-gauge-pointer circle{fill:#000}.ui-dark .ui-gauge text{fill:#FFF}.ui-dark .ui-gauge-pointer path{fill:#FFF}.ui-dark .ui-gauge-pointer circle{fill:#FFF}
...
.ui-layout2-section{position:absolute;z-index:2}.ui-layout2-main{z-index:1}

.ui-livestats svg{border:1px solid #E0E0E0}.ui-livestats path{stroke-width:2px;fill:none}.ui-livestats line{stroke-width:1px;stroke:#F0F0F0}.ui-dark .ui-livestats svg{border-color:#404040}.ui-dark .ui-livestats line{stroke:#404040}
...
.ui-radiobuttonexpert{color:#000;font-size:14px;vertical-align:baseline;position:relative}.ui-radiobuttonexpert-label{margin-bottom:5px;font-size:12px;text-align:left}.ui-radiobuttonexpert-label-required:before{color:red;content:'*';margin-right:5px}.ui-large .ui-radiobuttonexpert-label{font-size:14px}.ui-radiobuttonexpert-container{}
...
COMPONENT('gauge','decimals:1;format:{0}%;text:1;colors:30 #8CC152,40 #EDBC5A,30 #DB3737;stroke:25',function(self,config,cls){var cls2='.'+cls,svg;function Donut(cx,cy,radius,data){function arcradius(cx,cy,radius,degrees){var radians=(degrees-90)*Math.PI/180.0;return{x:cx+(radius*Math.cos(radians)),y:cy+(radius*Math.sin(radians))}}var decimals=4,total=0,arr=[],beg=-90,end=0,count=0;for(var i=0;i<data.length;i++)total+=data[i].value;for(var i=0;i<data.length;i++){var item=data[i],tmp={},p=(((item.value+1)/total)*100).floor(2);count+=p;if(i===length-1&&count<100)p=p+(100-count);end=beg+((180/100)*p);tmp.index=i;tmp.value=item.value;tmp.data=item;var b=arcradius(cx,cy,radius,end);var e=arcradius(cx,cy,radius,beg);var la=0;tmp.d=['M',b.x.floor(decimals),b.y.floor(decimals),'A',radius,radius,0,la,0,e.x.floor(decimals),e.y.floor(decimals)].join(' ');arr.push(tmp);beg=end}return arr}self.redraw=function(){svg.empty();var colors=config.colors.split(',');var color=[],data=[],stroke=[],centerX=150,centerY=150,radius=120;for(var i=0;i<colors.length;i++){var c=colors[i].trim().split(' ');data.push({value:+c[0]});color.push(c[1]);stroke.push(c[2]?+c[2]:config.stroke)}var arr=Donut(centerX,centerY,radius,data);for(var i=0;i<arr.length;i++){var item=arr[i];svg.asvg('<g><path d="{0}" stroke="{1}" fill="none" stroke-width="{2}" /></g>'.format(item.d,color[i],stroke[i]))}config.text&&svg.asvg('<g><text x="150" y="105" font-size="30" text-anchor="middle"></text></g><g transform="translate(0,-10)" class="{0}-pointer"><path d="M150 20 L145 145 L155 145 Z" class="{0}-value" transform="rotate(0,150,150)" /><circle cx="150" cy="150" r="10"></circle></g>'.format(cls))};self.make=function(){self.aclass(cls);self.append('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 300 150"></svg>');svg=self.find('svg');self.redraw()};self.setter=function(value){if(value>100)value=100;else if(!value)value=0;var max=180,deg=((max/100)*value)-(max/2);self.find(cls2+'-value').stop().animate({diff:deg},{step:function(val){this.setAttribute('transform','rotate('+val+',150,150)')},duration:300});config.text&&(self.find('text')[0].textContent=config.format.format(value.format(config.decimals)))}});
...
COMPONENT('layout2','scrollbar:1;parent:window;autoresize:1;margin:0',function(self,config,cls){var top,bottom,left,right,main,cachesize,init=false;self.init=function(){ON('resize + resize2',function(){for(var i=0;i<M.components.length;i++){var com=M.components[i];if(com.name==='layout2'&&com.dom.offsetParent&&com.$ready&&!com.$removed&&com.config.autoresize)com.resize()}})};self.parse_number=function(value,total){var tmp=value.parseInt();return value.indexOf('%')===-1?tmp:((total/100)*tmp)};self.parse_size=function(el){var size=(el.attrd('size')||'').split(',').trim();var obj={lg:size[0]||'0'};obj.md=size[1]==null?obj.lg:size[1];obj.sm=size[2]==null?obj.md:size[2];obj.xs=size[3]==null?obj.sm:size[3];var keys=Object.keys(obj);var reg=/px|%/;for(var i=0;i<keys.length;i++){var key=keys[i],val=obj[key];if(!reg.test(val))obj[key]+='px'}return obj};self.parse_item=function(el){var scrollbar=el.attrd('scrollbar');var type=el.attrd('type');var item={};item.el=el;item.size=type?self.parse_size(el):null;item.type=type||'main';item.css={};item.scrollbar=scrollbar?scrollbar.parseConfig():null;if(item.scrollbar){var screl;if(item.scrollbar.selector){screl=el.find(item.scrollbar.selector)}else{var dom=el[0],div=document.createElement('DIV');while(dom.children.length)div.appendChild(dom.children[0]);dom.appendChild(div);$(div).aclass(cls+'-scrollbar');screl=$(div)}var opt={visibleY:item.scrollbar.visible||item.scrollbar.visibleY,orientation:'y'};item.scrollbarcontainer=screl;item.scrollbar.instance=SCROLLBAR(screl,opt);item.scrollbar.resize=function(h){var t=this;item.scrollbarcontainer.css('height',h-(t.margin||0));item.scrollbar.instance.resize()}}el.aclass(cls+'-section '+cls+'-'+type.replace('2',''));return item};self.parse_cache=function(tmp){return(tmp.left||0)+'x'+(tmp.top||0)+'x'+(tmp.width||0)+'x'+(tmp.height||0)+'x'};self.make=function(){self.find('> *').each(function(){var el=$(this);var type=el.attrd('type');switch(type){case'top':case'top2':top=self.parse_item(el);break;case'bottom':case'bottom2':bottom=self.parse_item(el);break;case'right':right=self.parse_item(el);break;case'left':left=self.parse_item(el);break;default:main=self.parse_item(el);break}});self.resize()};self.resize=function(){setTimeout2(self.ID,self.resize2,50)};self.show=function(type){if(isMOBILE){switch(type){case'left':left&&left.el.css('width',WW).rclass('hidden');right&&right.el.aclass('hidden');break;case'right':left&&left.el.aclass('hidden');right&&right.el.css({left:0,width:WW}).rclass('hidden');break;case'main':right&&right.el.aclass('hidden');left&&left.el.aclass('hidden');break}}else self.resize2()};self.resize2=function(){var parent=self.parent(config.parent);var d=WIDTH();var w=parent.width();var h=parent.height();var tmp=d+'_'+w+'x'+h;if(cachesize===tmp)return;var m=config['margin'+d]||config.margin||0;h-=m;cachesize=tmp;main.w=w;main.h=h;var sizetop=top?self.parse_number(top.size[d],h):0;var sizebottom=bottom?self.parse_number(bottom.size[d],h):0;var sizeleft=left?self.parse_number(left.size[d],w):0;var sizeright=right?self.parse_number(right.size[d],w):0;if(top){if(top.type==='top2'){top.css.left=sizeleft;top.css.width=w-sizeright-sizeleft}else{top.css.left=0;top.css.width=w}top.css.top=0;top.css.height=sizetop;tmp=self.parse_cache(top.css);if(tmp!==top.sizecache){top.sizecache=tmp;top.el.tclass('hidden',!sizetop);if(!sizetop)delete top.css.height;top.el.css(top.css);top.scrollbar&&top.scrollbar.resize(sizetop)}}if(bottom){if(bottom.type==='bottom2'){bottom.css.left=sizeleft;bottom.css.width=w-sizeright-sizeleft}else{bottom.css.left=0;bottom.css.width=w}bottom.css.top=h-sizebottom;bottom.css.height=sizebottom;tmp=self.parse_cache(bottom.css);if(tmp!==bottom.sizecache){bottom.el.tclass('hidden',!sizebottom);if(!sizebottom)delete bottom.css.height;bottom.sizecache=tmp;bottom.el.css(bottom.css);bottom.scrollbar&&bottom.scrollbar.resize(sizebottom)}}if(left){if(top&&top.type==='top')left.css.top=sizetop;else left.css.top=0;if(bottom&&bottom.type==='bottom')left.css.height=h-sizebottom;else left.css.height=h;if(top&&top.type==='top')left.css.height-=sizetop;left.css.left=0;left.css.width=sizeleft;tmp=self.parse_cache(left.css);if(tmp!==left.sizecache){left.el.tclass('hidden',!sizeleft);if(!sizeleft)delete left.css.width;left.sizecache=tmp;left.el.css(left.css);left.scrollbar&&left.scrollbar.resize(left.css.height)}}if(right){if(top&&top.type==='top')right.css.top=sizetop;else right.css.top=0;if(bottom&&bottom.type==='bottom')right.css.height=h-sizebottom;else right.css.height=h;if(top&&top.type==='top')right.css.height-=sizetop;right.css.left=w-sizeright;right.css.width=sizeright;tmp=self.parse_cache(right.css);if(tmp!==right.sizecache){right.el.tclass('hidden',!sizeright);if(!sizeright)delete right.css.width;right.sizecache=tmp;right.el.css(right.css);right.scrollbar&&right.scrollbar.resize(right.css.height)}}if(main){main.css.top=sizetop;main.css.left=sizeleft;main.css.width=w-sizeleft-sizeright;main.css.height=h-sizetop-sizebottom;tmp=self.parse_cache(main.css);if(tmp!==main.sizecache){main.sizecache=tmp;main.el.css(main.css);main.scrollbar&&main.scrollbar.resize(main.css.height)}}if(!init){self.rclass('invisible hidden');init=true}self.element.SETTER('*/resize')};self.resizescrollbars=function(){top&&top.scrollbar&&top.scrollbar.instance.resize();bottom&&bottom.scrollbar&&bottom.scrollbar.instance.resize();left&&left.scrollbar&&left.scrollbar.instance.resize();right&&right.scrollbar&&right.scrollbar.instance.resize();main&&main.scrollbar&&main.scrollbar.instance.resize()};self.resizescrollbar=function(type){if(type==='top')top&&top.scrollbar&&top.scrollbar.instance.resize();else if(type==='bottom')bottom&&bottom.scrollbar&&bottom.scrollbar.instance.resize();else if(type==='left')left&&left.scrollbar&&left.scrollbar.instance.resize();else if(type==='right')right&&right.scrollbar&&right.scrollbar.instance.resize();else if(type==='main')main&&main.scrollbar&&main.scrollbar.instance.resize()};self.scrolltop=function(type){if(type==='top')top&&top.scrollbar&&top.scrollbar.instance.scrollTop(0);else if(type==='bottom')bottom&&bottom.scrollbar&&bottom.scrollbar.instance.scrollTop(0);else if(type==='left')left&&left.scrollbar&&left.scrollbar.instance.scrollTop(0);else if(type==='right')right&&right.scrollbar&&right.scrollbar.instance.scrollTop(0);else if(type==='main')main&&main.scrollbar&&main.scrollbar.instance.scrollTop(0)}});

COMPONENT('livestats','width:500;height:100;axislines:20;max:0',function(self,config,cls){var cls2='.'+cls,cache={},colors={},peak={},paths,cachedsize,cachedwidth,cachedheight,cachedlines;self.readonly();function diagonal(x1,y1,x2,y2){return'M'+x1+','+y1+'C'+(x1&&x2?((x1+x2)/2):0)+','+y1+' '+(x1&&x2?((x1+x2)/2):0)+','+y2+' '+x2+','+y2}self.color=function(value){var hash=HASH(value,true);var color='#';for(var i=0;i<3;i++){var value=(hash>>(i*8))&0xFF;color+=('00'+value.toString(16)).substr(-2)}return color};self.configure=function(key,value){switch(key){case'colors':var tmp=value.split(',').trim();for(var i=0;i<tmp.length;i++){var kv=tmp[i].split('=').trim();colors[kv[0]]=kv[1];cache[kv[0]]&&cache[kv[0]].attr('stroke',kv[1])}break}};self.make=function(){self.aclass(cls);self.append('<svg><g class="{0}-axis"></g><g class="{0}-paths"></g></svg>'.format(cls));paths=self.find(cls2+'-paths');self.resizeforce();self.on('resize + resize2',self.resize)};self.resize=function(){setTimeout2(self.ID,self.resizeforce,300)};self.resizeforce=function(){var width=typeof(config.width)==='string'?self.parent(config.width).width():config.width||self.element.width();var height=typeof(config.height)==='string'?self.parent(config.height).height():config.height||self.element.height();var tmp=width+'x'+height;if(tmp===cachedsize)return;if(config.marginW)width-=config.marginW;if(config.marginH)height-=config.marginH;cachedwidth=width;cachedheight=height;cachedsize=tmp;self.find('svg').attr('viewBox','0 0 '+width+' '+height);cachedlines=config.axislines==='auto'?Math.ceil(width/20):config.axislines;var axisw=(width/cachedlines)>>0;var axis=self.find(cls2+'-axis');if(axis[0].children.length)axis.empty();for(var i=1;i<cachedlines;i++)axis.asvg('<line x1="{0}" y1="0" x2="{0}" y2="{1}"></line>'.format(axisw*i,height))};self.render=function(path,points,max,index){var h=cachedheight-14,builder=[],bar=Math.ceil(cachedwidth/(cachedlines/2));var pp=[];pp.push({x:-20,y:h});for(var i=0;i<10;i++){var val=points[i]||0,p=val&&max?Math.round((val/max)*100):0;var y=(p?(h-((h/100)*p)):h)+(index*2);pp.push({x:(i*bar)+2,y:y+12})}pp.push({x:cachedwidth+cachedlines,y:pp.last().y});for(var i=0;i<(pp.length-1);i++){var d=diagonal(pp[i].x,pp[i].y,pp[i+1].x,pp[i+1].y);builder.push(d)}path.attr('d',builder.join(' '))};self.setter=function(value){if(!value)return;var keys=Object.keys(value);var max=config.max,bars=(cachedlines/2)>>0;for(var i=0;i<keys.length;i++){var key=keys[i],val=value[keys[i]];if(!cache[key]){peak[key]=[];for(var j=0;j<bars;j++)peak[key].push(0);paths.asvg('<path stroke="{0}" data-id="{1}" />'.format(colors[key]||self.color(key),key));cache[key]=paths.find('[data-id="{0}"]'.format(key))}peak[key].shift();peak[key].push(val||0)}if(!max){for(var i=0;i<keys.length;i++){var key=keys[i];for(var j=0;j<peak[key].length;j++){var val=peak[key][j];if(max<val)max=val}}}for(var i=0;i<keys.length;i++){var key=keys[i];self.render(cache[key],peak[key],max,i)}}});
